...
const scoreMatch = (q, fs) => {
  const terms = (q || "").trim().split(/\s+/).filter(Boolean).map(t=>t.toLowerCase());
  if (!terms.length) return 1.0;
  const hay = fs.join(" ").toLowerCase();
  const hit = terms.filter(t => hay.includes(t)).length;
  return hit / terms.length;
};
async function loadRSS(url){
  const r = await undiFetch(url, { headers: { "User-Agent": "RSS-Supplement/1.0" }});
  const xml = await r.text();
  const obj = new XMLParser({ ignoreAttributes:false }).parse(xml);
  const items = obj?.rss?.channel?.item ?? [];
  const arr = Array.isArray(items) ? items : [items];
  return arr.map(it => ({
    title: txt(it.title), link: txt(it.link), pubDate: txt(it.pubDate), description: txt(it.description)
  }));
}
function extractISBN13(link){
  const d = onlyDigits(link || ""); if (d.length < 13) return null;
  const cand = d.slice(-13); return cand.length === 13 ? cand : null;
}
  ...
  try {
    ...
    for (const f of feeds) {
      ...
      for (const it of rss) {
        ...
        list.push({ feed:f, title:it.title, link:it.link, pubDate:it.pubDate, isbn13, match_score:m,
                    tag: f==="shinkan" ? "recent_release" : "forthcoming" });
      }
    }
    list.sort((a,b) => b.match_score - a.match_score || (new Date(b.pubDate) - new Date(a.pubDate)));
